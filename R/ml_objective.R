...
ml_objective <- function(gamma, MAF_input, all_tumors, progressions, gene, variant, specific_mut_rates, modifier=0) {
  ...
  for (tumor in tumors_with_pos_mutated) {
    this_sum <- 0
    current_tumor_progression = get_progression_number(progressions, tumor) # current progression as numeric in the order
    for(levels in 1:current_tumor_progression){
      if(levels == 1){
