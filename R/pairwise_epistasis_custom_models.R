...
#' @param model Set to "basic_epistasis" (default)  to use built-in
...
#' @param model Passed from ces_epistasis or ces_gene_epistasis. Set to "basic_epistasis" (default) to use built-in
#'   model of epistatic selection, or supply a custom function factory (see details).
...
pairwise_variant_epistasis = function(cesa, variant_pair, samples, conf, compound_variants = NULL, model = "default", lik_args = list()) {
  ...
  if (running_compound) {
    ...
    # 3-item lists: first item is the Unique_Patient_Identifier; second is baseline rates for v1; third for v2
    with_just_1 = lapply(as.list(all_rates[tumors_just_v1])[1:3], setNames, tumors_just_v1)
    with_just_2 = lapply(as.list(all_rates[tumors_just_v2])[1:3], setNames, tumors_just_v2)
    with_both = lapply(as.list(all_rates[tumors_with_both])[1:3], setNames, tumors_with_both)
    with_neither = lapply(as.list(all_rates[tumors_with_neither])[1:3], setNames, tumors_with_neither)
  }
  
  if(is(model, "character")) {
    model = tolower(model)
    model[model == 'default'] = 'basic_epistasis'
    if(length(model) != 1 || ! model %in% c("basic_epistasis")) {
      stop("model should specify a built-in selection model (i.e., \"default\") or a custom function factory.")
    } else {
      if (model == 'basic_epistasis') {
