...
preload_ref_data = function(ref_key) {
  ...
  if(ref_key %in% names(ref_set_dirs)) {
    data_dir = ref_set_dirs[ref_key]
  } else {
    stop("Unrecognized reference data source. Run list_ces_ref_sets() to see available reference data.", call. = F)
  }
  ...
  if (! ref_key %in% ls(.ces_ref_data)) {
    ...
    .ces_ref_data[[ref_key]][["RefCDS"]] = get_ref_data(data_dir, "RefCDS")
    .ces_ref_data[[ref_key]][["gr_genes"]] = get_ref_data(data_dir, "gr_genes")
    .ces_ref_data[[ref_key]][["genome"]] = bsg
    if(check_for_ref_data(data_dir, "generic_exome_gr")) {
      .ces_ref_data[[ref_key]][["default_exome"]] = get_ref_data(data_dir, "generic_exome_gr")
    }
  }
}
...
list_ces_covariates = function() {
  ref_sets = ls(.ces_ref_data)
  if (length(ref_sets) == 0) {
    cat("No covariates data available. Create a CESAnalysis first.\n")
    return(invisible())
  }
  for (ref_set in ref_sets) {
    cov_files = list.files(system.file(paste0("ref_sets/", ref_set, "/covariates/"), package = "cancereffectsizeR"))
    cov_files = gsub("\\.rds$", '', cov_files)
    
    if (length(cov_files) == 0) {
      cat(ref_set, ": (no covariate data available)\n", sep = "")
    } else {
      initial = paste0(ref_set, ": ")
...
list_ces_signature_sets = function() {
  ref_sets = ls(.ces_ref_data)
  if (length(ref_sets) == 0) {
    pretty_message("No signature data available. Create a CESAnalysis first.\n")
    return(invisible())
  }
  for (genome in ref_sets) {
    sig_files = list.files(system.file(paste0("ref_sets/", genome, "/signatures/"), package = "cancereffectsizeR"))
