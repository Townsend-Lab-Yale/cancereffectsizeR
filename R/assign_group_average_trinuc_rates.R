...
#' all SBS records from whole-exome and whole-genome MAF data and naively assigns these rates to all samples. 
#' This can be helpful if you do not have SBS mutational signatures available for your species, or if 
#' you want to assume that all samples share the same SBS mutational processes without relying on signatures.
#' Normally, if mutational signatures are available, it is better to use trinuc_mutation_rates().
#' 
...
assign_group_average_trinuc_rates = function(cesa) {
  ...
  # Take just SBS
  sbs_maf = cesa@maf[variant_type == "sbs"]
  
  # Remove all recurrent sbs (sbs appearing in more than one sample)
  duplicated_vec_first <- duplicated(sbs_maf[,.(Chromosome, Start_Position, Tumor_Allele)])
  duplicated_vec_last <- duplicated(sbs_maf[,.(Chromosome, Start_Position, Tumor_Allele)],fromLast=T)
  duplicated_vec_pos <- which(duplicated_vec_first | duplicated_vec_last)
  if (length(duplicated_vec_pos) > 0) {
    sbs_maf <- sbs_maf[-duplicated_vec_pos,]
  }
  ...
  trinuc = BSgenome::getSeq(bsg, sbs_maf$Chromosome, sbs_maf$Start_Position - 1, sbs_maf$Start_Position + 1, as.character = T)
