...
### preload_maf()
After acquiring somatic mutation data in [MAF format](https://docs.gdc.cancer.gov/Data/File_Formats/MAF_Format/#gdc-maf-format-v100),
start by running `preload_maf()`, a function that can catch potential problems in your data. Here is an example of preloading three MAF files:
```{r}
library(cancereffectsizeR)
library(ces.refset.hg19)
maf1 = preload_maf(maf = "my_maf_1.txt", refset = ces.refset.hg19)
maf2 = preload_maf(maf = "my_maf_2.txt", refset = ces.refset.hg19)
...
When running with ces.refset.hg19 reference data, preloaded MAFs get annotations that may help meet these guidelines. (See the `preload_maf()` reference for details.) In this example, we'll exclude mutations in repetitive regions and at sites of germline variation, with the exception of mutations at sites of known cancer signficance (COSMIC mutation census Tiers 1-3). Simultaneously, we'll combine the three MAF tables together and run a helper function `check_sample_overlap()`, to identify possible duplicate samples across data sources:
```{r tidy = T}
  # create named list of MAF tables
  all_mafs = list(maf1 = maf1, maf2 = maf2, maf3 = maf3)
  combined_maf = rbindlist(all_mafs, idcol = 'source')
