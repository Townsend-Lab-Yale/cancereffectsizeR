...
Let's visualize the most selected variants. The following ggplot code should be fairly adaptable for a variety of data sets.
```{r}
library(ggplot2)
...
![](../inst/tutorial/BRCA_effects.jpg)
_PIK3CA_ and _ESR1_ variants take many of the top spots. The deconvolution of mutation rate from selection has revealed that some relatively low-prevalence variants have high effects. Interestingly, according to dNdScv, mutation in _ESR1_ at the gene level was not significantly greater than neutral in the TCGA cohort. The incorporation of TGS data and cancereffectsizeR's assessment of selection at the variant level confirm _ESR1_'s importance.

### Cancer effects (sequential model)
(Coming soon!)

### More options
By default, all recurrent variants (variants with prevalence >1 in MAF data) are included in a ces_variant() run. The `variants` option can be used to limit inference to just certain variants, or to test variants with prevalences of 1 or even 0. Note that single-hit variants will tend to receive over-estimated cancer effects because the occurrence of any non-selected variant is inherently improbable.

For each variant site, only samples with coverage at the site will inform selection inference. You can further restrict which samples are used using the `samples` argument. This could be helpful to compare assess and compare selection in various groups (e.g., smoking vs. non-smoking).
...
Note that the effect sizes are smaller for PIK3CA here, since we essentially mixed in lots of lower-effect PIK3CA variants. While 4 samples have both AKT1 E17K and one of the PIK3CA variants, this result strengthens the case for AKT1 mutation reducing selection for PIK3CA mutation.

#### Gene-level epistasis
The convenience function `ces_gene_epistasis()` applies the same model as `ces_epistasis()` at the gene level: It takes all recurrent mutations in a set of genes to test for epistatic effects between pairs of genes. Here's an example:
```{r}
...
The output for AKT1/TP53 would be identical to the previous example, except in this run, a second AKT1 variant (with n = 2) is included. Mutational selection in the two oncogenes appears to be reduced after mutation in TP53, while loss of TP53 remains selected on a mutant PIK3CA background and becomes even more favored on a mutant AKT1 background.
