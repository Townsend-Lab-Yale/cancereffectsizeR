...
Regrettably, some Bioconductor releases contain versions of required packages with bugs that cause cryptic messages in cancereffectsizeR workflows, such as `!anyNA(m32) is not TRUE` or `Error in data.frame(…, check.names=F): arguments imply differing number of rows`. If you encounter these problems, the simplest solution is to upgrade your Bioconductor version:
```{R}
...
Here is a straightforward cancereffectsizeR workflow: We load TCGA LUAD data, calculate mutation rates,
and quantify selection. We also attribute selection to mutational signatures and find that mutations attributed to tobacco-associated signatures have disproportionately large selective effects.
...
p
p2 = readRDS(system.file("tutorial/LUAD_sig_effects.rds", package = 'cancereffectsizeR'))
p2
```
...
```{r eval = T, echo = F}
formattable::formattable(readRDS(system.file("tutorial/BRCA_site_rates_example.rds", package = "cancereffectsizeR")), digits = 3)
```
...
```{r}
plot_effects(cesa$selection$recurrents, group_by = 'gene', topn = 10,
             label_individual_variants = FALSE)
```

```{r eval = T, echo = F}
readRDS(paste0(tutorial_dir, '/BRCA_effects_in_top_genes.rds'))
```

### Step-specific selection
The default model assumes that each variant has a single cancer effect across all samples. When samples represent an ordered sequence of progression stages (for example, normal tissue and primary tumor, or primary tumor and metastasis), `ces_variant_step()` instead estimates one selection intensity per stage, jointly under a single likelihood model. This assumes that samples at each later stage passed through every earlier stage, and each variant analyzed must belong to exactly one gene. If your sample groups are independent subgroups rather than a progression, run `ces_variant()` separately on each group instead (see the `samples` argument, below).

The example output in this section comes from a separate analysis of esophageal squamous cell carcinoma (ESCC), not the breast cancer data used in the rest of this tutorial. Suppose `cesa` holds somatic variants from normal esophageal epithelium and from primary ESCC tumors, with a sample-level column `Pre_or_Pri` marking each sample as normal (`"Pre"`) or tumor (`"Pri"`).

First, assign samples to ordered stages, and estimate gene mutation rates at each stage. Because later-stage samples have passed through earlier stages, each stage's rate is cumulative; `stage_mutation_proportions()` converts these into the proportion of each gene's mutation rate that accumulates during each stage.
```{r}
...
The model divides each gene's final-stage (total) mutation rate among stages, so assign that rate to all samples before running `ces_variant_step()`. Here, we estimate gene-level effects by combining each gene's variants into a single compound variant (see [CompoundVariantSets](#epistasis-and-compoundvariantsets), below). Then, we test whether stage-specific effects fit better than a single shared effect with `step_selection_LRT()`, and plot the results.
```{r}
...
plot_effects_step(cesa$selection$step_effects, group_by = "variant_name", lrt = lrt,
...
See the [step-specific selection](step_specific_selection.html) article for more detail, including analyses with more than two stages and how noisy (non-monotonic) stage rate estimates are handled.
...
```{r eval = T, echo = F}
formattable::formattable(readRDS(paste0(tutorial_dir, '/variant_ep_example.rds')), digits = 3)
```
...
```{r eval = T, echo = F}
formattable::formattable(readRDS(paste0(tutorial_dir, '/comp_variant_ep.rds')), digits = 3)
```
...
```{r eval = T, echo = F}
formattable::formattable(readRDS(paste0(tutorial_dir, '/gene_ep_example.rds')), digits = 3)
```
...
```{r eval = T, echo = F}
readRDS(paste0(tutorial_dir, '/BRCA_epistasis_example.rds'))
```
