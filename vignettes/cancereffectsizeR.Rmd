...
When running with ces.refset.hg19 reference data, preloaded MAFs get annotations relevant to these guidelines. (See the `preload_maf()` reference for details.) In this example, we'll exclude mutations in repetitive regions and at sites of germline variation, with the exception of mutations at sites of known cancer signficance (COSMIC mutation census tiers 1-3). Simultaneously, we'll combine the three MAF data tables together and run a helper function `check_sample_overlap()`, to identify possible duplicate samples across data sources. Lastly, we save one analysis-ready MAF per original MAF.
```{r eval = F}
  ...
  # remove problematic records (not strictly necessary, but nice for saving clean final MAF files)
  combined_maf = combined_maf[is.na(problem)]
  
  # remove variants from repetitive/germline variant sites except for COSMIC Tier 1-3 sites
  combined_maf = combined_maf[(repetitive_region == F & germline_variant_site == F) | cosmic_site_tier %in% 1:3]
  
  # check for sample duplicates and sample contamination
  possible_dups = check_sample_overlap(combined_table)
  ...
  # Presumably, you will want to save these as your analysis-read MAF text files
