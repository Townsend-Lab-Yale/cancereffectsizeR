...
The convenience function `ces_gene_epistasis()` provides a simpler way to apply the model of `ces_epistasis()` at the gene level. The `variants` argument provides three options for which variants from each gene to include in the inference: "recurrent" uses all recurrent variants; "nonsilent" uses all coding variants that are non-synonymous or splice-site-disrupting; alternatively, you can supply your own table of variants.
