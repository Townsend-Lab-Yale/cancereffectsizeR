...
```{r eval = T, include = F}
print(readRDS(system.file("tutorial/BRCA_site_rates_example.rds", package = "cancereffectsizeR")))
```
...
By default, all recurrent variants (variants with prevalence > 1 in MAF data) will be included. (The `variants` option can be used to limit inference to just certain variants, or to test variants with prevalences of 1 or even 0. However, single-hit variants will tend to receive over-estimated cancer effects because the occurence of any non-selected variant is inherently very unlikely.)
