...
Start by loading required packages. If you are using a custom refset, be sure to load the compatible BSgenome package.
```{r}
  ...
  # ces.refset.hg19 uses the hg19 assembly
  library(BSgenome.Hsapiens.UCSC.hg19) 
```

Get gene information used by the refset of interest. Currently, the simplest method is to pull it from a CESAnalysis object:

```{r}
tmp = CESAnalysis(refset = "ces.refset.hg19")

# Get gene namds and gene IDs; remove version number suffixes for short IDs
refset_genes = rbindlist(lapply(tmp$reference_data$RefCDS, '[', c("gene_id", "gene_name")))
refset_genes[, short_gene_id := gsub('\\..*', '', gene_id)]
setkey(refset_genes, "short_gene_id")

# Get genomic intervals for each gene
gene_gr = tmp$reference_data$gene_ranges
```
...
The data file is large because it contains many tissue types. We'll use the sample attributes file to subset to lung samples.
```{r}
...
We repeat this process with cancer cell line RNA-Seq data from DepMap's CCLE (Cancer Cell Line Encyclopedia). Again,
the data file contains many tissue types, and we subset to lung samples.
