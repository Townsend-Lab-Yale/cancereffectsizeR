...
SEE https://github.com/Townsend-Lab-Yale/cancereffectsizeR/blob/master/user_guide/cancereffectsizeR_user_guide.md FOR USER GUIDE
...
```{r adding unique tumor samples, eval=F}
LGG_MAF <- unique_tumor_addition_function(MAF.file = LGG_MAF)
```


```{r adding tumor allele, eval=F}
...
```{r effect size calculation,warning=F, eval=F}
...
```{r explore selection output all_mutations, eval=F}
...
```{r explore selection complete, eval=F}
